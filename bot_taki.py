def count_playable_cards (hand,pile,pile_color):
    ...
    for card in hand:
        if card["color"] == pile_color or card["color"] == "ALL":
            if card["value"] == "TAKI":
                how_many_TAKI += 1
            else:
                if card["value"] == "+":
    ...
    return cards_that_can_play,how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num
...
def the_color_with_the_largest_amount_of_cards(hand):
    ...
    for card in hand:
        if card["color"] != "ALL":
            cards_color_count[card["color"]] += 1
...
def more_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, cards_playing, order):
    """
...
def plain_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, pile_color):
    """
        ...
        for card in cards_that_can_play:
            if card["value"] == "TAKI":
                if card["color"] != "ALL":
                    ...
                    if len(cards_playing) == 0:
                        if len(cards_that_can_play) > 1:
                            cards_playing.append(card)
        if len(cards_playing) > 0:
            for card in cards_that_can_play:
                if card["value"] != "TAKI" and card["color"] == pile_color :
                    cards_playing.append(card)
            order = "close taki"
        else:
            cards_playing, order = more_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, cards_playing, order)
    else:
        cards_playing, order = more_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, cards_playing, order)
    return cards_playing, order
...
def turn (hand, after_me, before_me, pile, pile_color, before_me_last_turn):
    ...
    *this is the main function of the bot*
    ...
    cards_that_can_play,how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num = count_playable_cards(hand,pile,pile_color)
                                 
    #if someone used +2 on us                               
    if pile["value"] == "+2":
        if before_me - before_me_last_turn <= 1:
            cards_playing, order = plain_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, pile_color)
        else:
            if how_many_STOP_or_plus_two > 0:
                for card in cards_that_can_play:
                    if card["value"] == "+2":
    ...
    #the strategy can be found in taki.docs as a well explained flowchart
        ...
        if how_many_plus > 0  or how_many_CHDIR > 0  or  how_many_STOP_or_plus_two > 0 or  how_many_CHCOL > 0 or  how_many_regular_num > 0 or how_many_TAKI > 0:      
            if before_me < 4:
                if how_many_STOP_or_plus_two > 0:
                    optional_cards = find_correct_card(cards_that_can_play,["+2", "STOP"])
                    cards_playing.append(optional_cards[0])
                else:
                    if how_many_CHDIR > 0 and before_me > after_me:
                        optional_cards = find_correct_card(cards_that_can_play,["CHDIR"])
                        cards_playing.append(optional_cards[0])
                    else:
                        cards_playing, order = plain_strat (hand, how_many_TAKI, how_many_plus, how_many_CHDIR, how_many_STOP_or_plus_two, how_many_CHCOL, how_many_regular_num, cards_that_can_play, pile_color)    
